...
| `wf-b-test-runner` | `0E5R5Wr7slLbkqtP` | вспомогательный — Manual trigger → Execute Workflow → WF-B с тестовым payload. Используется только при разработке; перед сдачей в прод — архивировать. |
| `wf-c-test-runner` | `2hqo9MVxdxwEVmPd` | вспомогательный — то же самое для WF-C. Перед прод — архивировать. |
| `wf-d-test-runner` | `Qr4U9RTRDPwQC7Nj` | вспомогательный — то же самое для WF-D, переключатель `mode = 'within' / 'late'` в Test Payload для проверки обоих веток NPS. Перед прод — архивировать. |
...
| 2026-04-28 | WF-B (Welcome) реализован с встроенным dry-run: Receive Order → Config → Load Order State → Should Send? (idempotency + delivery) → Phone Valid? → Fetch Order Details → Format Welcome → Dry Run? → (Log Mock Send или Real WA Send stub) → Mark Welcome Sent. Stub реальной WA-отправки бросает ошибку — case insurance, чтобы случайно не активировать без замены. Создан `wf-b-test-runner` (Manual trigger → Execute Workflow → WF-B) для синтетических прогонов: тестовый payload в теле runner'а. Тест на order 64488 успешен — собрано 9-позиционное сообщение со списком и адресом, в `notification_log` записано `success=false, error_text='dry_run'`, идемпотентность подтверждена (повторный run не дублирует). |
| 2026-04-28 | WF-C (InTransit) реализован: Receive Status Change → Config → Load State → Should Send? → Fetch Details → Resolve Zone (TBD-2: heuristic chain delivery.zone_id → ... → shop.id fallback) → Zone Found? → Get Delivery Time (onError continueRegularOutput) → Have ETA? → Format InTransit / Mark Transit Skipped → Alert Zone Missing (WF-E). Дополнительный test-runner `wf-c-test-runner` (id `2hqo9MVxdxwEVmPd`). Тест на order 64488: zone resolved через `shop.id_fallback` = 4 → API вернул HTTP 404 «No query results for DeliveryZone 4» → корректно ушло в skip-ветку, `transit_skipped=TRUE` + Telegram-алерт админу. **Эмпирика: на турецком стенде зоны 1-10 не настроены вовсе** — пока админ не сконфигурирует зоны DataFood-side, сообщения «в пути» отправляться не будут (это AC-7, ожидаемое поведение). Happy-path тест WF-C валидируется при появлении настроенной зоны. |
| 2026-04-28 | **Уточнена safety-policy**: оказалось, что `Execute Workflow` node на современном n8n требует target `active: true`. Жёсткое «все WF неактивны» делало невозможным даже dev-тесты межсервисных вызовов. Новое правило (PLAN.md §8.1): WF-A (Schedule trigger, самотриггерится) — `active: false` до готовности; WF-B/C/D/E (только Execute Workflow trigger) — `active: true` безопасно (callable, но не самозапускаются). WF-E активирован для работы alert-цепочки. |
| 2026-04-28 | WF-D (NPS) реализован с встроенным dry-run и Wait-нодой. Receive Order Completed → Config (computes target = completed_at + nps_delay_minutes; cutoff = today 22:00 Europe/Istanbul) → Within Window? → Schedule Send (Postgres feedback_scheduled_at) → Wait Until Target → Re-Check State → Still Valid? (last_status='completed' AND feedback_sent_at IS NULL) → Format NPS → Dry Run? → Log Mock / Real WA stub → Mark Feedback Sent. В late-ветке (target>22:00): Mark Feedback Skipped с записью target в feedback_scheduled_at для отчётности. Создан `wf-d-test-runner` (id `Qr4U9RTRDPwQC7Nj`) с переключателем `mode='within'/'late'` для тестов обеих веток. Тесты на order 64488: (a) within — completed_at=NOW-119мин → target=NOW+1мин → Wait отработал ровно ~60с → проснулся → Format NPS дал «Здравствуйте! Заказ #64488 был доставлен. Поделитесь, пожалуйста, впечатлениями» → Log Mock + Mark Feedback Sent; (b) late — completed_at=21:30 Istanbul → target=23:30 → за окном → feedback_skipped=TRUE без отправки. Активированы WF-B, WF-C, WF-D (callable-only, без Schedule). |
| 2026-04-28 | WF-A связан с WF-B/C/D через Execute Workflow. Добавлены: Welcome Gate (после Insert New Order) → Run WF-B; InTransit Gate (после Update Status, regex по russian-словам «достав\\|курьер в пути\\|курьер выехал» + delivery) → Run WF-C; Completed Gate (после Update Final, is_completed AND delivery) → Run WF-D с completed_at=NOW. Все три gate'а делают early-return на pickup и не-доставочные статусы. Тест на синтетическом order_id=88888888: order_closed путь полностью отработал (Compute Diff → Switch → Confirm Closed (404) → Parse Final (canceled) → Update Final → Completed Gate FALSE → WF-D не вызвалась — корректно). Прямые тесты в WF-B/C/D через runner'ы пройдены раньше. |
| 2026-04-29 | Добавлен сервис `dashboard` (services/dashboard/): Node.js + Express + Tailwind UI + whatsapp-web.js. Экспонирует `POST /api/wa/send` для n8n (auth `X-Internal-Token`) и UI для подключения WhatsApp по QR. Развёрнут на сервере `72.56.92.175` в `/docker/sushimarket/`, контейнер `sushimarket-dashboard` Up healthy на `127.0.0.1:8080`. Парольная пара UI: `Admin` / `sushiadminmarket`. `DASHBOARD_INTERNAL_TOKEN=4380affd...59cb4` сгенерирован deploy-агентом, лежит на сервере в .env (chmod 600). Для развёртывания добавлен Dockerfile (node:22-alpine + Chromium), docker-compose.yaml в корне репо, package-lock.json. WhatsApp ещё не подключён — нужен QR-скан в UI. n8n WF-B/C/D ноды отправки ещё переписать на HTTP Request → dashboard (см. _TASK-FOR-PM-N8N.md). |
| 2026-04-29 | docker-compose.yaml: подключение к external network `n8n_n8n_network`, чтобы n8n-контейнер мог достучаться до dashboard по DNS-имени `sushimarket-dashboard:8080`. До этого n8n и dashboard были в разных сетях, любой HTTP Request падал с timeout. Это «Вариант A» из _TASK-FOR-PM-N8N.md §1 — постоянное решение, переживает рестарты. |
| 2026-04-29 | WhatsApp подключён через wa-bridge: QR отсканирован, статус `ready`, сессия в `/app/data/wa-session/`. Понадобились фиксы: переход с alpine на `node:22-bookworm-slim` + Chromium из apt; `shm_size: 1gb` в compose; убрать флаги `--single-process`/`--no-zygote`; auto-clean SingletonLock/Cookie/Socket при старте wa.js. Smoke-test `curl /api/wa/send` → реальное сообщение доставлено. |
| 2026-04-29 | Сшивка n8n ↔ dashboard завершена. В n8n credential `Dashboard Internal Token` (id `B05luGLJmm46wVY8`, header `X-Internal-Token`). В WF-B/C/D stub-ноды заменены на HTTP Request → `http://sushimarket-dashboard:8080/api/wa/send` с body `{phone, text, order_id, template}`. Dashboard `/api/wa/send` теперь автоматически пишет в `notification_log` (success/fail) — единый аудит. End-to-end тест runner-b с `DRY_RUN=false`: WF-B → /api/wa/send → wa-bridge → реальный WhatsApp на номер 79000430011 — доставлено. После теста `DRY_RUN` возвращён в `true` (operator redirect остался активен, страховочный режим). |
