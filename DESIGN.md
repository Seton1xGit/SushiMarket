...
## 12.1 Dry-run mode (safety gate)

**Зафиксировано пользователем 2026-04-28:** во время разработки **никаких** реальных сообщений клиентам. Все WhatsApp-отправки в WF-B / WF-C / WF-D — в режиме dry-run.

Реализация dry-run в каждом message-sending workflow:

```
[Build Message Payload]
  → [IF: $env.META_DRY_RUN === 'true']
       ├─ TRUE  → [Postgres INSERT notification_log
       │            (channel='whatsapp', success=false, error_text='dry_run', payload=<...>)]
       └─ FALSE → [WhatsApp Cloud API send] → [Postgres INSERT notification_log (success=true)]
```

Один env-флаг `META_DRY_RUN` (по умолчанию `true`) переключает все 3 потока. Перед production-запуском — пользователь меняет на `false` и проверяет в течение первого часа на ограниченном объёме.

**Активация workflow** (`workflow publish`) — отдельная отдельная ручная операция, выполняется **только** по явному запросу пользователя. По умолчанию все WF создаются с `active: false`.
